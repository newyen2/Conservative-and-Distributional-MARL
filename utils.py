...
# 從離散動作集隨機採樣，重複500次並儲存至資料集
# 動作現在只包含 select_action；target_point 由 Environment 根據 select_action 自動設定。
def collect_random(env, U, dataset, steps=500):
    ...
    for _ in range(steps):
        ...
        for u in range(U):
            action_vec = np.asarray([actions[u]], dtype=np.float32)
...
# 動作維度現在是 num_UAVs，每台 UAV 只儲存一個 select_action。
def prep_dataloader(state_dim, dataset, num_UAVs, batch_size=256):
    total_action_dim = num_UAVs
