...
def Train_MA_CIQL(model, device_coord, risky_region, alpha):
    # 初始化RNG, 環境與超參數
    config = get_config()
    rng = RNGManager()
    ...
    for u in range(config.U):
        agent = CQLAgent(state_size = env.nObservation,
    ...
    for epoch in range(1, config.epochs + 1):
        ...
        for _, experience in enumerate(dataloader):
            ...
            for u in range(config.U):            
                loss[u] = agents[u].Learn_CQL_ind((states, actions[:,[u]], rewards, next_states, dones))

        # personalized_fedavg_shared_layers(agents)

        # for u in range(config.U):
        #     agents[u].soft_update(agents[u].network, agents[u].target_network)

        # print(agents[0].network.state_dict())
