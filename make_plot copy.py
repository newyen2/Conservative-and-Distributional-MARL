...
result = []
for i in sub:
    r = (pd.read_csv(config.PATH+fr'\Result_Datas\Result_Offline_{i}_16%_2UAVs_300pen.csv').iloc[:, 0] / 1000).tolist()
    result.append(r)
...
for i in range(len(sub)):
    ...
    plt.plot(Episode_Reward_smooth_10, label=f'ξ = {sub[i]}', color = setting[i][0], 
