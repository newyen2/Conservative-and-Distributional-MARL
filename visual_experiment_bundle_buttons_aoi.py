# visual_experiment_bundle_buttons_aoi.py
...
# 5. 不顯示 move 箭頭
#    改為：將「最新一步位移（上一個 step -> 目前 step）」用不同顏色高亮
# 6. Device 的點大小以 AOI 表示
#
# 安裝需求：
#   pip install pandas pyarrow matplotlib numpy
#
# 使用範例：
#   python visual_experiment_bundle_buttons_aoi.py --episode_parquet episode.parquet --step_parquet step.parquet --episode 1 --mode interact
#   python visual_experiment_bundle_buttons_aoi.py --episode_parquet episode.parquet --step_parquet step.parquet --episode 1 --mode both
#   python visual_experiment_bundle_buttons_aoi.py --episode_parquet episode.parquet --step_parquet step.parquet --episode 1 --mode interact --traj_window 10
#
...
def compute_device_sizes(device_aoi, min_size=40, max_size=220):
    """
    將 device AOI 映射到 scatter size
    若 AOI 全相同，則回傳固定大小
    """
    aoi = flatten_numbers(device_aoi)
    ...
    if aoi_max == aoi_min:
        ...
        return [mid for _ in aoi]

    sizes = [
        min_size + (v - aoi_min) / (aoi_max - aoi_min) * (max_size - min_size)
        for v in aoi
    ]
    return sizes
...
def draw_common_scene(
    ...
    if device_points:
        ...
        if device_aoi is not None:
            sizes = compute_device_sizes(device_aoi)
            if len(sizes) != len(device_points):
                sizes = [70] * len(device_points)
        else:
            sizes = [70] * len(device_points)

        ax.scatter(dev_x, dev_y, s=sizes, marker="o", alpha=0.8, label="Device")

        if show_labels:
            aoi_values = flatten_numbers(device_aoi) if device_aoi is not None else []

            for idx, (x, y) in enumerate(device_points, start=1):
                label = f" D_{idx}"
                if show_aoi_text and idx - 1 < len(aoi_values):
                    label = f" D_{idx}:{aoi_values[idx - 1]:.0f}"
                ax.text(float(x), float(y), label, fontsize=8, va="bottom")
...
class EpisodeStepViewer:
    def __init__(
        ...
        self.show_aoi_text = show_aoi_text
    ...
    def draw_recent_traj_and_last_move(self, uav_points):
    ...
    def draw_frame(self):
        ...
        draw_common_scene(
            ...
            show_aoi_text=self.show_aoi_text,
        )

        self.draw_recent_traj_and_last_move(uav_points)
...
def main():
    ...
    parser.add_argument("--show_aoi_text", action="store_true", help="是否同時在 Device 標籤上顯示 AOI")
    parser.add_argument(
        "--select_base",
        type=str,
        default="auto",
    ...
    if args.mode in ["interact", "both"]:
        viewer = EpisodeStepViewer(
            ...
            show_aoi_text=args.show_aoi_text,
        )
        viewer.show()
