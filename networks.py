...
class Hybrid_SAC_Actor(nn.Module):
    def __init__(self, state_size, discrete_action_size, layer_size):
...
class Hybrid_SAC_Critic(nn.Module):
    def __init__(self, state_size, discrete_action_size, layer_size):
        ...
        input_size = state_size + discrete_action_size

        self.head_1 = nn.Linear(input_size, layer_size)
        self.ff_1 = nn.Linear(layer_size, layer_size)
        self.ff_2 = nn.Linear(layer_size, 1)

    def forward(self, state, discrete_action_onehot):
        x = torch.cat(
            [state, discrete_action_onehot],
            dim=-1,
        )
